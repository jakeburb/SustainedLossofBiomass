###################################################################
#                     RV SURVEY TOTAL BIOMASS                     #
###################################################################
#       Author: N. Rolland with updates from J.Burbank            #
###################################################################
...
##Comue indicies with Herring Correction
...
if (length(sp) > 1){
  ...
  set_sp_adj$alltaxa.weight.caught <- rowSums(set_sp_adj[,rows_weight.caught])
  set_sp_adj$alltaxa.number.caught <- rowSums(set_sp_adj[,rows_number.caught])
  
  smean_set_sp_adj_weight <- smean(set_sp_adj, var = "alltaxa.weight.caught", by = "year")
  smean_set_sp_adj_number <- smean(set_sp_adj, var = "alltaxa.number.caught", by = "year")
...
excel(smean_set_sp_adj_weight)
excel(smean_set_sp_adj_number)
## Plot

output <- file.path(fp, paste0("RV survey total abundance indices v4b All taxa ",
...
plot(smean_set_sp_adj_weight$year, smean_set_sp_adj_weight$mean, xaxs = "i", yaxs = "i", xaxt = 'n', yaxt = 'n', bty = 'n',
    ...
    mtext("All taxa", side = 3, line = -1.5, font = 2, cex = 1.5)
    
    dev.off()
    
################Commercial Fish Only#################
    
    ##Compute indicies with Herring Correction
    sp <- species_list$Cat_species_RV
    oo <- which(cat$species %in% sp)
    cat_sp <- cat[oo,]
    ...
    if (length(sp) > 1){
      ...
      set_sp_adjCOM$weight.caughtCOM <- rowSums(set_sp_adjCOM[,rows_weight.caughtCOM])
      set_sp_adjCOM$number.caughtCOM <- rowSums(set_sp_adjCOM[,rows_number.caughtCOM])
      
      smean_set_sp_adj_weightCOM <- smean(set_sp_adjCOM, var = "weight.caughtCOM", by = "year")
      smean_set_sp_adj_numberCOM <- smean(set_sp_adjCOM, var = "number.caughtCOM", by = "year")
...
excel(smean_set_sp_adj_weightCOM)    
excel(smean_set_sp_adj_numberCOM)    
## Plot
...
mergedALLfishcatchNCOM$weight.caughtNCOM <- rowSums(mergedALLfishcatchNCOM[,rows_weight.caughtNCOM])
mergedALLfishcatchNCOM$number.caughtNCOM <- rowSums(mergedALLfishcatchNCOM[,rows_number.caughtNCOM])
    
#Stratified mean for all fish
smean_set_sp_adj_weightNCOM <- smean(mergedALLfishcatchNCOM, var = "weight.caughtNCOM", by = "year")
smean_set_sp_adj_numberNCOM <- smean(mergedALLfishcatchNCOM, var = "number.caughtNCOM", by = "year")
...
excel(smean_set_sp_adj_weightNCOM)
excel(smean_set_sp_adj_numberNCOM)
...
###########Pelagic#############
    ##Comute Indicies with Herring Correction##
    sp <- species_list$Cat_species_RV
    oo <- which(cat$species %in% sp)
    cat_sp <- cat[oo,]
    ...
    if (length(sp) > 1){
      ...
      set_sp_adjP$weight.caughtP <- rowSums(set_sp_adjP[,rows_weight.caughtP])
      set_sp_adjP$number.caughtP <- rowSums(set_sp_adjP[,rows_number.caughtP])
      
      smean_set_sp_adj_weightP <- smean(set_sp_adjP, var = "weight.caughtP", by = "year")
      smean_set_sp_adj_numberP <- smean(set_sp_adjP, var = "number.caughtP", by = "year")
...
###Mean, min and max of stratified biomass by weight and number over time series for all fish
#Weight
...
excel(smean_set_sp_adj_weightP)
excel(smean_set_sp_adj_numberP)
    ## Plot
...
#Sum weight caught and number caught for all fish
mergedALLfishcatchD$weight.caughtD <- rowSums(mergedALLfishcatchD[,rows_weight.caughtD])
mergedALLfishcatchD$number.caughtD <- rowSums(mergedALLfishcatchD[,rows_number.caughtD])
    
#Stratified mean for all fish
smean_set_sp_adj_weightD <- smean(mergedALLfishcatchD, var = "weight.caughtD", by = "year")
smean_set_sp_adj_numberD <- smean(mergedALLfishcatchD, var = "number.caughtD", by = "year")
...
excel(smean_set_sp_adj_weightD)   
excel(smean_set_sp_adj_numberD)
    ## Plot
...
###Mean, min and max of total biomass by weight and number over time series for lobster
#Weight
...
excel(smean_set_sp_adj_weightL)  
excel(smean_set_sp_adj_numberL)
    ## Plot
...
#    postscript(output)
    ...
    #Sum weight caught and number caught for Crabs
    mergedALLfishcatchC$weight.caughtC <- rowSums(mergedALLfishcatchC[,rows_weight.caughtC])
    mergedALLfishcatchC$number.caughtC <- rowSums(mergedALLfishcatchC[,rows_number.caughtC])
    
    #Stratified mean Crabs
    smean_set_sp_adj_weightC <- smean(mergedALLfishcatchC, var = "weight.caughtC", by = "year")
    smean_set_sp_adj_numberC <- smean(mergedALLfishcatchC, var = "number.caughtC", by = "year")
    
excel(smean_set_sp_adj_weightC)  
excel(smean_set_sp_adj_numberC)
      ## Plot
...
##########Shrimps################
    #Merge shrimp catch cards
    mergedALLfishcatchS<-merge.catch(set, catch.card=adjustedcatchcards, species = sp_shrimps)
    
    #Specify rows with weight caugh , numbers(abundance) caughtt is incosistentely recorded, only report weight(biomass)
    rows_weight.caughtS <- grep("weight.caught", names(mergedALLfishcatchS))

    
    #Sum weight caught and number caught for shrimps
    mergedALLfishcatchS$weight.caughtS <- rowSums(mergedALLfishcatchS[,rows_weight.caughtS])

    
    mergedALLfishcatchS$weight.caughtS[mergedALLfishcatchS$year %in% c(1971:1979)] <- 0

    
    #Stratified mean for shrimps
    smean_set_sp_adj_weightS <- smean(mergedALLfishcatchS, var = "weight.caughtS", by = "year")
    ...
    excel(smean_set_sp_adj_weightS)
