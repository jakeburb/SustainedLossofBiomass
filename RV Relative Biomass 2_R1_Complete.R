...
##Pull good data and correct where needed
...
#correct for database error for shrimp with duplicates in 2004 and 2016 for species 2414, for now remove the line with the lowest catch
...
##Stacked Bar Plot not presented in paper but used for visualization

output <- file.path(fp, paste0("Relative Biomass Bar Plot (all species) with crab and lobster (update Feb 2023) - ", min(years), "-", max(years), ".png"))
...
output <- file.path(fp, paste0("Relative Biomass (selected species) with crab and lobster (update Feb 2023) ", min(years), "-", max(years), ".png"))
...
excel(spec_selected_straplot)
######

setEPS()

output <- file.path(fp, paste0("Relative Biomass (selected species) with crab and lobster (update Feb 2023) - ", min(years), "-", max(years), ".eps"))
...
##################R1 ADDITIONS#####################################
#############Create Supplementary Relative Biomass Figure based on reviewer comments###############

my.perc.mat <- my.mat / year.vec * 100
rowSums(my.perc.mat) 
##Remove un-examined species
columns_to_remove <- c("642", "230", "611", "251", "2520") 
my.perc.mat <- my.perc.mat[, !colnames(my.perc.mat) %in% columns_to_remove]
# Select Taxa with relative abundance between 0.17 and 3.3 %
mx <- apply(my.perc.mat, 2, max)
spec_selected_straplot <- my.perc.mat[, mx >= .17 & mx < 3.3]
colnames(spec_selected_straplot) <- c("Haddock","Pollock","Witch Flounder", "Striped Atlantic Wolffish","American Shad","Alewife", "Greenland Cod", "Cunnner", "Thorny Skate", 
                                      "Little Skate","Winter Skate", "Longhorn Sculpin", "Shorthorn Sculpin", "Mailed Sculpin", "Sea Raven", "Lumpfish", "Northen Sandlance", 
                                      " Laval's Eelpout", "Ocean Pout", "Arctic Eelpout", "Vahl's Eelpout", "Atlantic Rock Crab", "Toad Crab") 
...
output <- file.path(fp, paste0("Relative Biomass of Other Species- Supplementary Figure 1 - ", min(years), "-", max(years), ".png"))
...
x <- strat.plot(spec_selected_straplot, yvar = as.numeric(rownames(spec_selected_straplot)),
                scale.percent = TRUE, graph.widths = 1, minmax = NULL, scale.minmax = TRUE,
                xLeft = 0.07, xRight = 0.95, yBottom = 0.075, yTop = 0.84,
                title = "", cex.title = 1.8,
                y.axis = TRUE, x.axis = TRUE, min.width = 5, ylim = NULL, y.rev = FALSE,
                y.tks = seq(min(years), max(years), 2), ylabel = "", cex.ylabel = 1.5, cex.yaxis = 0.9,
                xSpace = 0.008, x.pc.inc = 4, x.pc.lab = TRUE, x.pc.omit0 = TRUE,
                wa.order = "bottomleft", plot.line = TRUE,
                col.line = "black", lwd.line = 1, plot.bar = TRUE,
                lwd.bar = 1, col.bar = "darkgrey", sep.bar = FALSE, bar.back = FALSE,
                plot.poly = TRUE, col.poly = "#7570B340", col.poly.line = NA,
...
mtext("Relative biomass (%)", side = 1, line = 3.8, cex = 1.3, font = 2)
mtext("Year", side = 2, adj = 0.45, padj = -4.4, cex = 1.3, font = 2)
abline(h = c(1989), col = "red", lwd = 1, lty = 1)
abline(h = c(1979, 2004, 2009), col = "red", lwd = 1, lty = 2)
